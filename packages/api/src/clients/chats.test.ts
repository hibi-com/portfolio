...
describe("chats client", () => {
	...
	describe("markMessagesAsRead", () => {
		describe("正常系", () => {
			test("メッセージを既読にする", async () => {
				...
				expect(mockChatsMarkMessagesAsRead).toHaveBeenCalledWith("room-123", body, { participantId: "part-123" });
			});
		});
	});
	...
	describe("chats オブジェクト", () => {
		...
		test("sendMessageメソッドが正しく動作する", async () => {
			...
			const result = await chats.sendMessage("room-123", { chatRoomId: "room-123", content: "Hi", participantId: "part-123" });
