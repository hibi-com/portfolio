...
export const meta: MetaFunction = () => {
    return [{ title: "E2E Test Reports - Test Portal" }];
};

export async function loader({ request }: LoaderFunctionArgs) {
    const projects = getAllProjects("e2e");
...
export default function E2EReports() {
    ...
    return (
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
            <h1 style={{ fontSize: "2rem", marginBottom: "2rem" }}>🎭 E2E Test Reports</h1>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                                {projectReports.map((report) => (
                                    ...
                                    >
                                        ...
                                        {report.metadata.summary && "passed" in report.metadata.summary && (
                                            <div style={{ fontSize: "0.875rem", color: "#6b7280" }}>
                                                ✅ {report.metadata.summary.passed} / ❌{" "}
                                                {report.metadata.summary.failed} / ⏭️ {report.metadata.summary.skipped}
                                            </div>
                                        )}
                                    </Link>
                                ))}
