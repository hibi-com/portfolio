...
const InquiryStatusSchema = z
    .enum(["OPEN", "IN_PROGRESS", "WAITING_CUSTOMER", "RESOLVED", "CLOSED"])
    .openapi("InquiryStatus");
const InquiryPrioritySchema = z.enum(["LOW", "MEDIUM", "HIGH", "URGENT"]).openapi("InquiryPriority");
const InquiryCategorySchema = z
    .enum(["GENERAL", "SUPPORT", "SALES", "BILLING", "TECHNICAL"])
    .openapi("InquiryCategory");
...
const CreateInquiryInputSchema = z
    .object({
        customerId: z.string().uuid().optional(),
        assigneeId: z.string().uuid().optional(),
        subject: z.string().min(1),
...
const UpdateInquiryInputSchema = z
    .object({
        customerId: z.string().uuid().optional(),
        assigneeId: z.string().uuid().optional(),
...
const AddResponseInputSchema = z
    .object({
        inquiryId: z.string().uuid(),
        userId: z.string().uuid().optional(),
        content: z.string(),
