...
const listLogsRoute = createRoute({
	...
	responses: {
		200: { content: { "application/json": { schema: z.array(EmailLogSchema) } }, description: "List of email logs" },
	},
});
...
const listTemplatesRoute = createRoute({
	...
	responses: {
		200: { content: { "application/json": { schema: z.array(EmailTemplateSchema) } }, description: "List of templates" },
	},
});
...
const updateTemplateRoute = createRoute({
	...
	request: { params: IdParamSchema, body: { content: { "application/json": { schema: UpdateTemplateInputSchema } } } },
	responses: {
...
const listLogsHandler: Handler<{ Bindings: Env }> = async (c) => {
	try {
		...
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to fetch email logs", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const getLogHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to fetch email log", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const listTemplatesHandler: Handler<{ Bindings: Env }> = async (c) => {
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to fetch templates", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const getTemplateHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to fetch template", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const createTemplateHandler: Handler<{ Bindings: Env }> = async (c) => {
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to create template", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const updateTemplateHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to update template", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const deleteTemplateHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to delete template", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const sendEmailHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to send email", { originalError: error instanceof Error ? error : new Error(String(error)) });
