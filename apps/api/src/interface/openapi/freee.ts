...
// Schemas
const FreeeIntegrationSchema = z.object({
	id: z.string().uuid(),
...
const callbackRoute = createRoute({
	...
	responses: {
		200: { content: { "application/json": { schema: FreeeIntegrationSchema } }, description: "Integration created" },
		400: { content: { "application/json": { schema: ErrorResponseSchema } }, description: "Validation error" },
	},
});
...
const getIntegrationRoute = createRoute({
	...
	responses: {
		200: { content: { "application/json": { schema: IntegrationStatusSchema } }, description: "Integration status" },
	},
});
...
const getAuthUrlHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const container = new DIContainer(c.env.DATABASE_URL, c.env.CACHE_URL, undefined, undefined, c.env.FREEE_AUTH_BASE_URL, c.env.FREEE_API_BASE_URL);
		...
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to generate auth URL", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const callbackHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to complete OAuth flow", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const getIntegrationHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to fetch integration", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const disconnectHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to disconnect", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const syncImportHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to sync from freee", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const syncExportHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to sync to freee", { originalError: error instanceof Error ? error : new Error(String(error)) });
...
const getSyncLogsHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const logger = getLogger();
		const appError = error instanceof AppError ? error : AppError.fromCode(ErrorCodes.INTERNAL_SERVER_ERROR, "Failed to fetch sync logs", { originalError: error instanceof Error ? error : new Error(String(error)) });
