...
// Schemas using @hono/zod-openapi's z
...
const getCustomerHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const duration = (Date.now() - startTime) / 1000;
		metrics.httpRequestDuration.observe({ method: "GET", route: "/api/crm/customers/:id", status: "200" }, duration);
...
const updateCustomerHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const duration = (Date.now() - startTime) / 1000;
		metrics.httpRequestDuration.observe({ method: "PUT", route: "/api/crm/customers/:id", status: "200" }, duration);
...
const deleteCustomerHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const duration = (Date.now() - startTime) / 1000;
		metrics.httpRequestDuration.observe({ method: "DELETE", route: "/api/crm/customers/:id", status: "204" }, duration);
...
// Register routes with handlers (using type assertion for PoC)
// biome-ignore lint/suspicious/noExplicitAny: PoC - type safety will be improved in Phase 3
