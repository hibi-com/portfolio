...
const CreatePipelineInputSchema = z.object({
	name: z.string().min(1),
	description: z.string().optional(),
	isDefault: z.boolean().optional(),
}).openapi("CreatePipelineInput");

const UpdatePipelineInputSchema = z.object({
...
const getPipelineHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const duration = (Date.now() - startTime) / 1000;
		metrics.httpRequestDuration.observe({ method: "GET", route: "/api/crm/pipelines/:id", status: "200" }, duration);
...
const updatePipelineHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const duration = (Date.now() - startTime) / 1000;
		metrics.httpRequestDuration.observe({ method: "PUT", route: "/api/crm/pipelines/:id", status: "200" }, duration);
...
const deletePipelineHandler: Handler<{ Bindings: Env }> = async (c) => {
	...
	try {
		...
		const duration = (Date.now() - startTime) / 1000;
		metrics.httpRequestDuration.observe({ method: "DELETE", route: "/api/crm/pipelines/:id", status: "204" }, duration);
