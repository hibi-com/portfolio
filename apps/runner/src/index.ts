...
import { parseDurationMs, type PodMessage, type RunRecord, type RunnerConfig, validateConfig } from "./types";
...
async function handlePod(env: Env, message: PodMessage): Promise<void> {
    ...
    const batch = run.mode === "load" ? await runLoadBatch(run) : await runDastBatch(run);
