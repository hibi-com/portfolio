...
- **出力先**: `apps/scenario/public/reports/coverage/{project}/`（Test Portalで表示）
...
主なコマンド: `bun run test`（Small）, `bun run integration`（Medium）, `bun run scenario`（シナリオ / Large）, `bun run e2e`（Browser Run E2E）, `bun run coverage`。  
...
カバレッジレポートは `apps/scenario/public/reports/coverage/` に出力される。Test Portal（`apps/scenario` で `bun run dev`）の `/coverage` で確認可能。  
...
## シナリオテスト（apps/scenario）と E2E（apps/e2e）

- **シナリオテスト**（`apps/scenario`）: リポジトリ内 Playwright。ローカル / CircleCI。Worker デプロイ不要。
- **E2E**（`apps/e2e`）: Cloudflare Browser Run からデプロイ済みアプリへアクセス。詳細は `apps/e2e/README.md`。

旧「独立 E2E」パッケージはシナリオテストへリネーム済みです。

実行: `apps/scenario` で `bun run scenario:web` など。
...
実行: `apps/scenario` で `bun run accessibility`。  
詳細は `package.json` を参照。
...
実行: `apps/scenario` で `bun run monkey`。  
詳細は `package.json` を参照。
