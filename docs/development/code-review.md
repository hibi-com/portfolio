...
| レベル | 該当条件 | 例 | 対応 |
| ------ | -------- | -- | ---- |
| **🔴 クリティカル** | ・セキュリティリスク<br>・アーキテクチャ違反<br>・N+1クエリ<br>・テスト未達<br>・型エラー | SQLインジェクション、レイヤー逆転、N+1、Branches < 100% | **即修正必須**。これがあるとマージ不可。 |
| **🟠 重要** | ・品質低下<br>・保守性問題<br>・パフォーマンス懸念<br>・ドキュメント不足 | DRY違反、エラーハンドリング不足、シーケンス図未更新 | **修正推奨**。3件以上で要修正判定。 |
| **🟡 軽微** | ・可読性向上<br>・スタイル改善<br>・将来的改善提案 | 変数名改善、コメント追加、リファクタリング提案 | 任意。次回対応可。 |
