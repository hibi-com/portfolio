...
**問題**: Cloudflare D1 またはローカル libSQL への接続が失敗する。

**解決方法**:

**本番（Workers）**: D1 はバインディング（`DB`）経由で接続します。`wrangler.toml` / Pulumi で作成した D1 の ID・名前が正しいか、Cloudflare Dashboard でデータベースが存在するか確認してください。接続文字列の `DATABASE_URL` は本番 Workers では通常使いません。

**ローカル**: libSQL（sqld）は Docker Compose の `sqlite` サービスです。ホストからは `http://127.0.0.1:8081`、コンテナ間は `http://sqlite:8080` を使用します。`docker compose up sqlite setup-db` で起動とマイグレーションを確認してください。

**エッジケース**:

- ローカルは libSQL（`http://sqlite:8080` / ホストでは `http://127.0.0.1:8081`）、本番 Workers は Cloudflare D1 バインディングです。
- Docker の DB volume が古い場合は `docker volume rm portfolio_sqlite_data` のあと `docker compose up setup-db sqlite` で作り直します。
...
ローカル開発環境では、Docker Compose の `setup-db`（`.docker/db/compose.yml`）が SQLite にマイグレーションとシードを適用します。  
マイグレーションは `packages/db/migration/` にあり、`.docker/db/migration` からシンボリックリンクで参照します。  
本番 D1 への適用は Wrangler（`wrangler d1 migrations apply`）または CI 経由で行います。
...
マイグレーションファイルは `packages/db/migration/` に配置され、Docker Compose 起動時に `setup-db` が適用します。  
スキーマ変更後は volume を消して `setup-db` を再実行するか、新しいマイグレーションを追加してください。

**エッジケース**:

- ドメイン分割されたPrismaスキーマ（`packages/db/prisma/schema/`配下の複数ファイル）を使用しているため、スキーマ変更時は全ファイルの整合性を確認してください。
- ローカル SQLite と本番 D1 は同じ Prisma `provider = "sqlite"` スキーマを共有します。MySQL 固有構文は使えません。
...
アーティファクト取得エラーの場合、CIジョブでのビルドが成功しているか、CircleCI Artifacts / workspace に成果物が保存されているか確認します。  
...
D1 / SQLite は日時を UTC 相当（Unix epoch / ISO 文字列）で扱います。  
アプリケーション側でJST（日本標準時、UTC+9）に変換する必要があります。  
Prismaスキーマで`DateTime`型を使用している場合、JavaScriptの`Date`オブジェクトはローカルタイムゾーンを使用します。  
一貫性のため、全ての日時データをUTCで保存し、表示時のみローカルタイムゾーンに変換することを推奨します。Cloudflare Workersは常にUTCで動作するため、`Date.now()`の結果はUTCです。
...
SQLite / D1 はテキストを UTF-8 として扱い、絵文字を含む Unicode をサポートします。  
...
- 絵文字や特殊 Unicode は SQLite の TEXT 型でそのまま保存できます。クライアント側のエンコーディング不一致に注意してください。
...
- トランザクション内での長時間処理は避けてください。D1 / SQLite のステートメントタイムアウトや Workers の CPU 時間制限に注意してください。
- Cloudflare Workersの同時接続数は限られています。D1 は接続プールではなくバインディング経由のため、長時間のトランザクションや大量の逐次クエリを避けてください。
